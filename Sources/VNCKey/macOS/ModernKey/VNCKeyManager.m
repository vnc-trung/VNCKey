...
@interface VNCKeyManager ()

@end

@implementation VNCKeyManager {
...
+(void)showUpdateMessage:(NSWindow*)parent needUpdating:(BOOL)needUpdating newVersion:(NSString*)versionString {
    NSAlert *alert = [[NSAlert alloc] init];
    [alert setMessageText:(needUpdating ? [NSString stringWithFormat:@"VNC-Key Có phiên bản mới (%@), bạn có muốn cập nhật không?", versionString] : @"Bạn đang dùng phiên bản mới nhất!")];
    [alert setInformativeText:(needUpdating ? @"Bấm 'Có' để cập nhật VNC-Key." : @"")];
...
+(void)launchUpdateHelper {
    ...
    NSString* target = [NSString stringWithFormat:@"%@/VNCKeyUpdate.app", [self getApplicationSupportFolder]];
...
+(NSString*)getApplicationSupportFolder {
    ...
    return [NSString stringWithFormat:@"%@/VNCKey", applicationSupportDirectory];
}

+(NSString*)getUpdateBundlePath {
    NSString *currentpath = [[NSBundle mainBundle] bundlePath];
    return [NSString stringWithFormat:@"%@/Contents/Library/LoginItems/VNCKeyUpdate.app", currentpath];
}
@end
