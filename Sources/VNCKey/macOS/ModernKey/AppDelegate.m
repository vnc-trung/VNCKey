...
-(void)askPermission {
    ...
    [alert setMessageText: [NSString stringWithFormat:@"VNC-Key cần bạn cấp quyền để có thể hoạt động!"]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    if ([runningApp containsObject:VNCKEY_BUNDLE]) { //if already running -> exit
        [NSApp terminate:nil];
        return;
    }
    ...
    if (!dontCheckUpdate)
        [VNCKeyManager checkNewVersion:nil callbackFunc:nil];
...
-(void)setRunOnStartup:(BOOL)val {
    CFStringRef appId = (__bridge CFStringRef)@"com.vietnamchess.VNCKeyHelper";
...
-(void)onConvertTool {
    ...
    //[VNCKeyManager showDockIcon:YES];
...
-(void)onQuickConvert {
    if ([VNCKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [VNCKeyManager showMessage: nil message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [VNCKeyManager showMessage: nil message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}

-(void) onControlPanelSelected {
    if (_mainWC == nil) {
        _mainWC = [[NSStoryboard storyboardWithName:@"Main" bundle:nil] instantiateControllerWithIdentifier:@"VNCKey"];
    }
    //[VNCKeyManager showDockIcon:YES];
...
#pragma mark Reset VNC-Key after mac computer awake
-(void)receiveWakeNote: (NSNotification*)note {
    [VNCKeyManager initEventTap];
}

-(void)receiveSleepNote: (NSNotification*)note {
    [VNCKeyManager stopEventTap];
}
...
-(void)activeAppChanged: (NSNotification*)note {
    if (vUseSmartSwitchKey && [VNCKeyManager isInited]) {
        OnActiveAppChanged();
    }
}
