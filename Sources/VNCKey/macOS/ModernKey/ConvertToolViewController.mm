...
-(void)fillData {
    NSArray* codeData = [VNCKeyManager getTableCodes];
...
- (IBAction)onConvertButton:(id)sender {
    if ([VNCKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [VNCKeyManager showMessage: self.view.window message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [VNCKeyManager showMessage: self.view.window message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}
