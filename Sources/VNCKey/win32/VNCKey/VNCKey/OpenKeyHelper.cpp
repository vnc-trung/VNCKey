...
static LPCTSTR sk = TEXT("SOFTWARE\\Vietnamchess\\VNCKey");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "VNCKeyProject";
...
int CF_OPENKEY = RegisterClipboardFormat(_T("VNCKey Format"));
...
void OpenKeyHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			...
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn VNC-Key /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("VNC-Key"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("VNC-Key"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn VNC-Key /f", SW_HIDE);
	}
}
...
string& OpenKeyHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("VNC-Key64.exe")) == 0 ||
		wcscmp(_exeName, _T("VNC-Key32.exe")) == 0 || 
		wcscmp(_exeName, _T("OpenKey64.exe")) == 0 ||
...
wstring OpenKeyHelper::getContentOfUrl(LPCTSTR url){
	...
	wsprintf(path, TEXT("%s\\_VNCKey.tempf"), path);
