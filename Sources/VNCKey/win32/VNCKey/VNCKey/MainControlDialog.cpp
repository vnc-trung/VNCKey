...
Fanpage: mailto:vnc@vietnamchess.vn
...
static LPCTSTR _terminalSpeedLabels[] = {
    _T("Nhanh nhất"),
    _T("Nhanh"),
    _T("Bình thường"),
    _T("Chậm"),
...
void MainControlDialog::initDialog() {
    ...
    // hUpdateButton removed in VNC-Key
    ...
    // checkCheckNewVersion removed in VNC-Key
    // hUpdateButton removed in VNC-Key
    ...
    // --- PATCH: Terminal speed slider ---
    sliderTerminalSpeed = GetDlgItem(hTabPage3, IDC_SLIDER_TERMINAL_SPEED);
    labelTerminalSpeedVal = GetDlgItem(hTabPage3, IDC_STATIC_TERMINAL_SPEED_VAL);
    SendMessage(sliderTerminalSpeed, TBM_SETRANGE, TRUE, MAKELPARAM(0, 4));
    SendMessage(sliderTerminalSpeed, TBM_SETTICFREQ, 1, 0);
    SendMessage(sliderTerminalSpeed, TBM_SETPAGESIZE, 0, 1);
    // --- END PATCH ---
...
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    // --- PATCH: Handle terminal speed slider ---
    case WM_HSCROLL: {
        if ((HWND)lParam == sliderTerminalSpeed) {
            int pos = (int)SendMessage(sliderTerminalSpeed, TBM_GETPOS, 0, 0);
            if (pos >= 0 && pos <= 4) {
                APP_SET_DATA(vTerminalSpeed, pos);
                SetWindowText(labelTerminalSpeedVal, _terminalSpeedLabels[pos]);
            }
        }
        break;
    }
...
void MainControlDialog::fillData() {
    ...
    // SendMessage(checkCheckNewVersion, ...) removed in VNC-Key
    ...
    // --- PATCH: Terminal speed slider ---
    if (vTerminalSpeed < 0) vTerminalSpeed = 0;
    if (vTerminalSpeed > 4) vTerminalSpeed = 4;
    SendMessage(sliderTerminalSpeed, TBM_SETPOS, TRUE, vTerminalSpeed);
    SetWindowText(labelTerminalSpeedVal, _terminalSpeedLabels[vTerminalSpeed]);
    // --- END PATCH ---
...
void MainControlDialog::onUpdateButton() {
    ...
    if (OpenKeyManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("VNC-Key Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("VNC-Key Update"), MB_OK);
    }
...
void MainControlDialog::requestRestartAsAdmin() {
    ...
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại VNC-Key để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại VNC-Key không?"),
            _T("VNC-Key"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
